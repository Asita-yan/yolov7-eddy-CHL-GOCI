import numpy as np
import torch
from torchvision.ops import nms, batched_nms, _batched_nms_vanilla
import torchvision.ops.boxes as tob
...
class DecodeBox():
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4,eddy_minradius=100):
        #----------------------------------------------------------#
        ...
        for i, image_pred in enumerate(prediction):
            ...
            # detections_class = output
            #
            # # ------------------------------------------#
            # #   使用官方自带的非极大抑制会速度更快一些！
            # #   筛选出一定区域内，属于同一种类得分最大的框,先对每一种类非极大抑制，再一起非极大抑制。
            # # ------------------------------------------#
            keep = batched_nms(
                detections[:, :4],
            ...
            each_detections=max_detections[nms(
                max_detections[:, :4],
                max_detections[:, 4] * max_detections[:, 5],
                # max_detections[:, -1],
                nms_thres
            )]

            # Add max detections to outputs
            output[i] = each_detections if output[i] is None else torch.cat((output[i], each_detections))
            ...
            if output[i] is not None:
                ...
                output[i][:, :4]    = self.yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image)
                delete_index=[]
                for num in range(output[i].shape[0]):
                    # 去除识别边框在边界位置的分割结果
                    if (output[i][num][:4]<=5).sum() or (output[i][num][:4]>=635).sum():
                    ...
                    if area>1/4*np.pi*(eddy_minradius/0.5)**2:   #100为识别的涡旋最小半径km
                        ...
                        #     output[i]=None
                        #     continue
                output[i] = np.delete(output[i], np.array(delete_index,dtype=int), axis=0)
                if output[i].shape[0]==0:
                    output[i]=None
        return output
...
if __name__ == "__main__":
    ...
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        img = Image.open("D:\\PC\\yolov7-pytorch-master\\img\\street.jpg").resize([640, 640])
